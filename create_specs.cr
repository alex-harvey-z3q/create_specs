...
def default_config
  return [ENV["HOME"], "config.yml"].join("/")
end

def parse_arguments
  options = Config.from_yaml(File.read(default_config))

  OptionParser.parse! do |parser|
    parser.banner = "Usage: #{File.basename(PROGRAM_NAME)} [options]"
    parser.on("-f", "--config_file CONFIG", "Path to config file") do |opt|
      options = Config.from_yaml(File.read(opt))
    end
    parser.on("-c", "--catalog CATALOG", "Path to the catalog JSON file") do |opt|
      options.catalog_file = opt
    end
    parser.on("-C", "--class CLASS", "Class (or node) name under test") do |opt|
      options.class_name = opt
    end
    parser.on("-o", "--output OUTPUTFILE", "Path to the output Rspec file") do |opt|
      options.output_file = opt
    end
    parser.on("-x", "--exclude RESOURCE", [
      "Resources to exclude. String or Regexp. ",
      "Repeat this option to exclude multiple resources"].join) do |opt|
      options.excludes << opt
    end
    parser.on("-i", "--include RESOURCE",
      "Resources to include despite the exclude list.") do |opt|
      options.excludes.delete(opt)
    end
    parser.on("-I", "--only-include RESOURCE",
    ...
    parser.on("-t", "--[no-]compile-test", "Include or exclude the catalog compilation test") do |opt|
      options.compile_test = true
    end
